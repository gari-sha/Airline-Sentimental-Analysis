...
wf_df$sentiment <- mydata$airline_sentiment
...
accuracy <- sum(y_true[-training_index] == y_hat)/ length(y_hat)
accuracy

lr <- table(y_hat, true = y_true[-training_index])
lr
### Random forest #######
...
accuracy <- sum(y_true[-training_index] == y_hat_rf)/ length(y_hat_rf)
accuracy
...
accuracy <- sum(y_true[-training_index] == y_hat_naive)/ length(y_hat_naive)
accuracy
